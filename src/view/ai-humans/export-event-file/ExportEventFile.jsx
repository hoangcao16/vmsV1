...
const ExportEventFile = () => {
    ...
    useEffect(() => {
        ...
        if (per) {
            ...
            Notification({
                ...
                description: `${t('noti.do_not_have_premission_to_view_list_event')}`
            });
        }
        ...
        try {
            ...
            if (per) {
                ...
                Notification({
                    type: NOTYFY_TYPE.warning,
                    title: `${t('noti.archived_file')}`,
                    description: `${t('noti.do_not_have_premission_to_action')}`
                });
                setListFiles([]);
                setTotal(0);
            }
    ...
    const editFileHandler = async (file, dataList, perStr) => {
        ...
        if (per) {
            ...
            if (response) {
                Notification({type: NOTYFY_TYPE.success, title: `${t('noti.archived_file')}`, description: `${t('noti.successfully_edit_file')}`});
