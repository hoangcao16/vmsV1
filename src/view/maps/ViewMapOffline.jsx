    ...
    try {
      ...
      if (!mapboxRef.current) {
        ...
        mapboxRef.current.on('load', () => {
          // console.log('mapCamMarkersRef.current', mapCamMarkersRef.current)
          // let features = []
          // mapCamMarkersRef.current.map((item, index) => {
          //   let data = {
          //     type: 'feature',
          //     geometry: {
          //       type: 'Point',
          //       coordinates: [item._lngLat.lng, item._lngLat.lat]
          //     }
          //   }
          //   features.push(data)
          // })
          ...
          mapboxRef.current.on('click', 'clusters', (e) => {
            const features = mapboxRef.current.queryRenderedFeatures(e.point, {
              layers: ['clusters']
            });
            const clusterId = features[0].properties.cluster_id;
            mapboxRef.current.getSource('earthquakes').getClusterExpansionZoom(
              clusterId,
              (err, zoom) => {
                if (err) return;
          ...
          mapboxRef.current.on('click', 'unclustered-point', (e) => {
        ...
        //   mapboxRef.current.addSource('market', {
        //     type: 'geojson',
        //     // Point to GeoJSON data. This example visualizes all M1.0+ earthquakes
        //     // from 12/22/15 to 1/21/16 as logged by USGS' Earthquake hazards program.
        //     data: data,
