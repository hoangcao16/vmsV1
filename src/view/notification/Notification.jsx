...
const Notification = (props) => {
  ...
  const renderText = (cellValue) => {
    ...
    return <Text style={{ maxWidth: "100%" }}>{cellValue}</Text>;
  };
  ...
  const listItems = props?.notif.map((n, index) => (
    <li key={index} className={`notif_list ${n?.isSeen ? "seen" : ""}`}>
      ...
      <div className="notif_list__content--body">
        {n?.type === "WARNING_DISK"
          ? `  ${t("noti.hard_drive")} ${
              !isEmpty(n?.name) ? n?.name : `${t("noti.donot_have")}`
            } ${t("noti.is_used")}
        ${
          !isEmpty(n?.percentUsed)
            ? `${n?.percentUsed}%`
            : `${t("noti.donot_have")}`
        }. ${t("noti.estimated_remaining_usable_time_is")}
        ${
          !isEmpty(n?.estimatedTime)
            ? n?.estimatedTime
            : `${t("noti.donot_have")}`
        }.`
          : `Camera ${n?.name} ${
              n?.status ? `${t("noti.active")}` : `${t("noti.inactive")}`
            } `}
      </div>
    </li>
  ));
  ...
  return (
    <div
      className="notification__scroll--infinite__wrapper"
          ...
          >
            {props?.notif ? (
              <ul className="list__notification">
                {isEmpty(props.notif) ? (
                  <div className="list__notification--no_infor">
                    {t("noti.no_information")}
                  </div>
                ) : (
                  listItems
                )}
