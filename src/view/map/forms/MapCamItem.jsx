...
const MapCamItem = ({ camera, handleSelectCamera, isControlCam }) => {
  ...
  const camsLiveSelector = useSelector(
    (state) => state.map.camsLive.listCamLive
  );
  ...
  return (
    <>
      ...
      >
        <div className="camera-list__cam card-cam__item__content">
          <i
            className={
          ...
          <Tooltip
            ...
            title={() => (
              <div>
                <p>
                  {t("view.map.name")}: {camera.name}
                </p>
                <p>
                  {t("view.map.administrative_unit_uuid")}:{" "}
                  {camera.administrativeUnitName}
                </p>
                <p>
                  {t("view.map.zone")}: {camera.zoneName}
                </p>
              </div>
            )}
          >
        ...
        {isControlCam && showControlCam && (
          <div className="card-cam__item__control">
            {playCam ? (
              <PlayCamIcon onClick={handlePauseCam} />
            ) : (
              <PauseCamIcon onClick={handlePlayCam} />
            )}
          </div>
        )}
      </div>
