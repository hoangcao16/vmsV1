...
const TableEventFile = (props) => {
    ...
    const renderEvent = (row) => {
        ...
        if (row.isSaved && !row.editMode) return <Tooltip title={row.eventName}><span>{row.eventName}</span></Tooltip>
        return <Select
            allowClear
            showSearch
    ...
    const renderNote = (row) => {
        if (row.isSaved && !row.editMode) return <Tooltip title={row.note}><span>{row.note}</span></Tooltip>
        return (
            <Input
