...
const PlaybackSliderTime = ({ playbackByDragSlideTime, dateTimeSeek }) => {
  ...
  const [realtimeCurrentDateTime, setRealTimeCurrentDateTime] = useState(
    moment()
  );
  ...
  const getWidthBoxTimeline = () => {
    const bboxSliders = document
      .getElementById("slider__markers")
  ...
  function dragElement(elmnt) {
    ...
    const calculateSeekTimeTo = (offsetPixel, fromDateTime) => {
      ...
      if (distanceByPerMarker < 0) {
        currentTimeAfterSeek = moment(currentTimeAfterSeek).subtract(
          (distanceByPerMarker * zoomLevel) / 60,
          "h"
        );
      } else {
        currentTimeAfterSeek = moment(currentTimeAfterSeek).subtract(
    ...
    <div className="row slider-container">
      <div className="slider" id="slider">
        ...
        <div id="slider__markers" className="slider__markers">
          {renderMarkers()}
          <div id="slider__mask" className="slider__markers"></div>
        </div>
        <div className="slider__zoom">
          <span>
            {" "}
            <Minus style={{ cursor: "pointer" }} onClick={handleZoomIn} />
          </span>
          <span>
            {" "}
            <Plus style={{ cursor: "pointer" }} onClick={handleZoomOut} />
          </span>
        </div>
      </div>
    </div>
