                ...
                rules={[
                  ...
                  ({ getFieldValue }) => ({
                    validator(rule, value) {
                      ...
                      const [ip2_octet1, ip2_octet2, ip2_octet3, ip2_octet4] =
                        data.split(".").map((value) => Number(value));
                      const [ip1_octet1, ip1_octet2, ip1_octet3, ip1_octet4] =
                        getFieldValue(["p1"])
                          .split(".")
                      ...
                      if (data) {
                        if (!data.match(ipformat)) {
                          ...
                          return Promise.reject(
                            `IP kết thúc phải lớn hơn IP bắt đầu`
                          );
                        } else {
                          return Promise.resolve();
                        }
      ...
      {!isEmpty(listCamera) && (
        <List
          ...
          renderItem={(item) => {
            return (
              <List.Item
                className={`${item?.type === "other" ? "other" : ""} `}
              >
                {renderItem(item)}
              </List.Item>
            );
